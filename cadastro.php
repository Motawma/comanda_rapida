...
if (isLoggedIn()) {
    ...
    switch ($role) {
        case 'master': header('Location: master.php'); break;
...
<head>
  ...
  <title>Criar Conta — V12 Comandas</title>
  ...
  <style>
    ...
    .login-header { text-align: center; padding: 2rem 1.75rem 1rem; }
    .login-header h4 {
    ...
    .login-body { padding: 1.25rem 1.75rem 2rem; }
      ...
      <div class="login-body">
        ...
        <form id="cadastroForm" autocomplete="off">
          <div class="mb-3">
            ...
            <input id="nome" class="form-control" placeholder="Nome completo" required autocomplete="name">
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3 password-wrap">
            <label class="field-label" for="password2">Confirmar senha</label>
            <input id="password2" type="password" class="form-control" placeholder="Repita a senha" required minlength="6" autocomplete="new-password">
            <button type="button" class="btn-eye" onclick="togglePass('password2')">👁️</button>
          </div>
          <button type="submit" class="btn btn-entrar w-100">Criar Conta</button>
        </form>
...
document.getElementById('cadastroForm').addEventListener('submit', async (e) => {
  ...
  if (!nome || !email || !pass) {
    msg.innerHTML = '<div class="alert alert-warning py-2 small">Preencha todos os campos</div>';
    return;
  }
  if (pass !== pass2) {
  ...
  if (pass.length < 6) {
    msg.innerHTML = '<div class="alert alert-danger py-2 small">A senha deve ter no mínimo 6 caracteres</div>';
    return;
  }
  ...
  try {
    const res = await fetch('api/cadastro_publico.php', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ nome, email, password: pass })
    });
    const j = await res.json();

    if (j.success) {
      msg.innerHTML = '<div class="alert alert-success py-2 small">Conta criada! Redirecionando...</div>';
      setTimeout(() => {
        window.location = 'admin.php?onboarding=1';
      }, 1000);
    } else {
      msg.innerHTML = '<div class="alert alert-danger py-2 small">' + (j.message || 'Erro ao criar conta') + '</div>';
    }
  } catch (err) {
