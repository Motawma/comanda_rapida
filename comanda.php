...
<div id="bannerProntos">
  🔔 <strong>PRONTO para retirar:</strong> <span id="bannerProntosMesas"></span> — <span class="banner-count" id="bannerProntosCount">0</span> comanda(s) · toque para ir à comanda
</div>
      ...
      linhas.push(
        `<div class="resumo-linha">
          ...
          <span class="resumo-acoes">
            <button type="button" class="btn-resumo btn-resumo-minus" data-id="${id}" title="Diminuir">−</button>
            <span class="resumo-qty">${q}</span>
            <button type="button" class="btn-resumo btn-resumo-plus" data-id="${id}" title="Aumentar">+</button>
...
function removerItemResumo(id){
  id = Number(id);
  if (!id) return;
  // Zera inputs visíveis desse produto
  document.querySelectorAll(`.qty-input[data-id="${id}"]`).forEach(input => {
    input.value = '0';
  });
  // Remove do carrinho virtual
  if (window.carrinho && window.carrinho[id]) {
...
function editarQtdResumo(id, delta){
  id = Number(id);
  if (!id) return;
  // Usa carrinho virtual (única fonte)
...
document.getElementById('resumo').addEventListener('click', function(e){
  ...
  if (plus) { e.preventDefault(); editarQtdResumo(plus.dataset.id, +1); return; }
...
(() => {
  ...
  function abrirComandasFiltradoProntos() {
    ...
    if (modalEl && typeof bootstrap !== 'undefined') {
      ...
      modalEl.addEventListener('shown.bs.modal', function onShown() {
        modalEl.removeEventListener('shown.bs.modal', onShown);
        // Filtra o cache local para mostrar apenas comandas com itens PRONTO
        const prontas = comandasCache.filter(c =>
          (c.itens || []).some(it => (it.item_status || '') === 'PRONTO')
        );
        ...
        if (containerInner) {
          if (prontas.length > 0) {
            ...
            containerInner.innerHTML = `
              <div class="comandas-empty">
                <div class="icon">✨</div>
                <div class="fw-bold mb-1">Nenhuma comanda pronta</div>
  ...
  function atualizarBannerProntos(comandas) {
    ...
    const comandasComPronto = (comandas || []).filter(c =>
      (c.itens || []).some(it => (it.item_status || '') === 'PRONTO')
    );
  ...
  async function marcarItensEntregues(btn) {
    ...
    try {
      const res = await fetch('api/item_status_atualizar.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ item_ids: itemIds, status: 'ENTREGUE' })
      });
      const data = await res.json();
      if (data.ok) {
