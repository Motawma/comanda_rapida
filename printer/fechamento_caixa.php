...
if ($sessaoId <= 0) { echo 'Sessão inválida'; exit; }
...
$paidStmt = $pdo->prepare("SELECT COALESCE(SUM(total),0) AS v FROM pedidos WHERE caixa_sessao_id = ? AND status = 'PAGO'");
$paidStmt->execute([$sessaoId]); $total_pago = (float)$paidStmt->fetch()['v'];

$cancelStmt = $pdo->prepare("SELECT COALESCE(SUM(total),0) AS v FROM pedidos WHERE caixa_sessao_id = ? AND status = 'CANCELADO'");
$cancelStmt->execute([$sessaoId]); $total_cancelado = (float)$cancelStmt->fetch()['v'];

// Forma de pagamento e obrigatoria — sem NULL
$pfStmt = $pdo->prepare("SELECT UPPER(TRIM(COALESCE(metodo_pagamento,''))) AS metodo, COALESCE(SUM(total),0) AS total FROM pedidos WHERE caixa_sessao_id = ? AND status = 'PAGO' GROUP BY metodo_pagamento");
$pfStmt->execute([$sessaoId]);
...
function enviarWhatsApp(){
  ...
  const msg=[
    'Fechamento Caixa #<?= $sess['id'] ?>',
    'Dinheiro: '+fmt(<?= $por_forma['DINHEIRO'] ?>),
    'Pix: '+fmt(<?= $por_forma['PIX'] ?>),
    'Credito: '+fmt(<?= $por_forma['CREDITO'] ?>),
    'Debito: '+fmt(<?= $por_forma['DEBITO'] ?>),
    'Total: '+fmt(<?= $total_pago ?>),
  ].join('\n');
